...
{
    public class BuyIndex : BaseIndex
    {
        private SolrHelper<ModBuys> solrHelper = SolrHelper<ModBuys>.Instance(ECore.BuyCore);
        private ISolrOperations<ModBuys> solr = null;
        private DalBuy dal = null;
        ...
        public BuyIndex(LogMsg logMsg)
        {
            ...
            this.count = this.dal.GetCount(Convert.ToInt32(ConfigSetting.GetValue("LastBuyId")));
        }
        ...
        {
            List<ModBuys> list = this.dal.GetList();
            if (list == null)
            {
                this.count = 0;
                this.value = 0;
        ...
        private void CreateIndex(List<ModBuys> list, bool isCreate)
        {
            if (list != null)
            {
                foreach (var item in list)
                {
                    ...
                    ConfigSetting.SetValue("LastBuyId", item.Id);
                    logMsg.writeBarLog(++value, this.count);
                    logMsg.writeLog(item.Id, string.Concat("成功：", item.Title));
                }
                if (isCreate)
