...
{
    public class ProIndex : BaseIndex
    {
        ...
        public ProIndex(LogMsg logMsg)
        {
            ...
            this.count = this.dal.GetCount(Convert.ToInt32(ConfigSetting.GetValue("LastSellId")));
        }
        ...
        private void CreateIndex(List<ModProduct> list, bool isCreate)
        {
            if (list != null)
            {
                foreach (var item in list)
                {
                    ...
                    ConfigSetting.SetValue("LastSellId", item.Id);
                    logMsg.writeBarLog(++value, this.count);
                    logMsg.writeLog(item.Id, item.Title);
                }
                //if (isCreate)
