...
{
    public class ProBestIndex : BaseIndex
    {
        private SolrHelper<ModProduct> solrHelper = SolrHelper<ModProduct>.Instance(ECore.ProCore);
        private ISolrOperations<ModProduct> solr = null;
        private DalProBest dal = null;
        ...
        public ProBestIndex(LogMsg logMsg)
        {
            this.logMsg = logMsg;
            dal = new DalProBest();
            if (solr == null)
            ...
            this.count = this.dal.GetCount(Convert.ToInt32(ConfigSetting.GetValue("LastBestId")));
        }
        ...
        {
            List<ModProduct> list = this.dal.GetList();
            if (list == null)
            {
                this.count = 0;
                this.value = 0;
        ...
        private void CreateIndex(List<ModProduct> list, bool isCreate)
        {
            if (list != null)
            {
                foreach (var item in list)
                {
                    if (item.MemberGrade > 1)
                        this.solr.AddWithBoost(item, item.ProBoost);                                  //会员的产品增加相应的权重
                    else
                        this.solr.Add(item);
                    if (!isCreate)
                        this.solr.Commit();                                //逐条提交变更                
                    ConfigSetting.SetValue("LastBestId", item.Id);
                    logMsg.writeBarLog(++value, this.count);
                    logMsg.writeLog(item.Id, string.Concat("索引创建成功：", item.Title));
                }
                if (isCreate)
