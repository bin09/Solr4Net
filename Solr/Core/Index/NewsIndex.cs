...
{
    public class NewsIndex:BaseIndex
    {
        private SolrHelper<ModNews> solrHelper = SolrHelper<ModNews>.Instance(ECore.NewsCore);
        private ISolrOperations<ModNews> solr = null;
        private DalNews dal = null;
        ...
        public NewsIndex(LogMsg logMsg)
        {
            this.logMsg = logMsg;
            dal = new  DalNews();
            if (solr == null)
            ...
            this.count = this.dal.GetCount(Convert.ToInt32(ConfigSetting.GetValue("LastNewsId")));
        }
        ...
        {
            List<ModNews> list = this.dal.GetList();
            if (list == null)
            {
                this.count = 0;
                this.value = 0;
        ...
        private void CreateIndex(List<ModNews> list, bool isCreate)
        {
            if (list != null)
            {
                foreach (var item in list)
                {
                    ...
                    ConfigSetting.SetValue("LastNewsId", item.Id);
